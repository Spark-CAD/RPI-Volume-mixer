...
BASE_DIR            = os.path.expanduser('~/volume-controller')
...
def _bg_media_poll():
    ...
    while True:
        ...
        try:
            ...
            # Only update state + log when something meaningful actually changed
            changed = (
                new_title   != last_title or
                new_playing != state['media']['playing'] or
                new_dur     != state['media']['duration'] or
                new_source  != state['media']['source']
            )
            if changed:
                state['media'].update({
                    'title':    new_title,
                ...
                print(f"[Media] {new_title} [{'Playing' if new_playing else 'Paused'}] {int(new_dur)}s")
...
def _bg_volume_sync_loop():
    ...
    Sends a fingerprint of current known values so PC can reply 'unchanged' cheaply."""
    POLL_INTERVAL = 2.0
    _last_fingerprint = ''
    while True:
        ...
        try:
            pot_cfg = _load_json(POT_CONFIG_FILE, {str(i): '' for i in range(8)})
            # Build fingerprint of current Pi-side values so PC can skip if identical
            fingerprint = ','.join(str(state['pot_values'][i]) for i in range(8))
            r = requests.post(
                f'http://{pc_ip}:5001/api/get_volumes',
                json={'pot_config': pot_cfg, 'fingerprint': fingerprint},
                timeout=2
            )
            if r.status_code == 204:
                continue  # PC says nothing changed — skip processing entirely
            ...
            if not volumes:
                continue  # no apps with volume yet — silent
            now = time.time()
                ...
                if 0 <= ch < 8:
                    if now - _pot_last_moved[ch] > POT_LOCK_SECS:
                        ...
                        if state['pot_values'][ch] != new_val:
                            ...
                            print(f'[VolSync] ch{ch} <- {new_val}% (PC change)')
        except Exception as e:
